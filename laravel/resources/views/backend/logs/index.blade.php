...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="btn-group" role="group">
            ...
            <form action="{{ route('admin.logs.clear') }}" method="POST" class="d-inline" onsubmit="return confirm('Vider le fichier de log ?');">
                @csrf
                ...
                <div class="form-group mr-3 mb-2">
                    ...
                    <select name="level" id="level" class="form-control">
                        ...
                        @foreach ($levels as $key => $config)
                            <option value="{{ $key }}" {{ $level === $key ? 'selected' : '' }}>{{ ucfirst($key) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group mr-3 mb-2">
                    <label for="search" class="mr-2">Recherche</label>
                    <input type="text" name="search" id="search" class="form-control" value="{{ $search }}" placeholder="Message ou canal">
                </div>
                <div class="form-group mr-3 mb-2">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-{{ $levels[$log['level']]['color'] ?? 'secondary' }}">
                                            <i class="fa-solid fa-{{ $levels[$log['level']]['icon'] ?? 'circle-info' }}"></i>
                                    ...
                                    <td>
                                        <div>{{ $log['message'] }}</div>
                                        @if (!empty($log['extra']))
                                            <small class="text-muted d-block mt-1">{{ str_replace("\n", ' ', $log['extra']) }}</small>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
