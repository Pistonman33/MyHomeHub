...
class Categorie extends Model
{
    ...
    protected $fillable = ['nom', 'group', 'color'];
    ...
    public function getColor(){
        // Priorité à la couleur dans la DB, sinon utiliser la couleur calculée
        return $this->color ?? self::$colors[$this->id - 16] ?? '#999999';
    }

    static function getColorById($id){
        $categorie = self::find($id);
        return $categorie ? $categorie->getColor() : (self::$colors[$id - 16] ?? '#999999');
    }
